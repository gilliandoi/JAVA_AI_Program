...
public class Planner {
	...
	private Vector initGoalList() {
		...
		// Johnはお茶を飲む
		goalList.addElement("John drink tea");
		return goalList;
	}
	...
	private Vector initInitialState() {
		...
		// Johnは喉が乾いた
		initialState.addElement("John thirsty");
		// 冷蔵庫が空
		initialState.addElement("fridge empty");
		// ママがお店に行く
		initialState.addElement("mum goto store");
	...
	private void initOperators() {
		...
		// OPERATOR 1：お茶を飲む
		/// NAME
		String name1 = new String("?x drink ?y");
		/// IF
		Vector ifList1 = new Vector();
		ifList1.addElement(new String("?x thirsty"));
		ifList1.addElement(new String("?x holding ?y"));
		/// ADD-LIST
		Vector addList1 = new Vector();
		addList1.addElement(new String("?x drink ?y"));
		addList1.addElement(new String("Hand empty"));
		/// DELETE-LIST
		Vector deleteList1 = new Vector();
		deleteList1.addElement(new String("?x thirsty"));
		deleteList1.addElement(new String("?x holding ?y"));
		Operator operator1 = new Operator(name1, ifList1, addList1, deleteList1);
		operators.addElement(operator1);

		// OPERATOR 2：お茶を取る
		/// NAME
		String name2 = new String("?x holding ?y");
		/// IF
		Vector ifList2 = new Vector();
		ifList2.addElement(new String("?z has ?y"));
		/// ADD-LIST
		Vector addList2 = new Vector();
		addList2.addElement(new String("?x holding ?y"));
		addList2.addElement(new String("?z empty"));
		addList2.addElement(new String("?m goto ?n"));
		/// DELETE-LIST
		Vector deleteList2 = new Vector();
		deleteList2.addElement(new String("?z has ?y"));
		Operator operator2 = new Operator(name2, ifList2, addList2, deleteList2);
		operators.addElement(operator2);

		// OPERATOR 3：お店に行く
		/// NAME
		String name3 = new String("?m goto ?n");
		/// IF
		Vector ifList3 = new Vector();
		ifList3.addElement(new String("?z empty"));
		ifList3.addElement(new String("?m goto ?n"));
		/// ADD-LIST
		Vector addList3 = new Vector();
		addList3.addElement(new String("?m buy ?y for ?z"));
		/// DELETE-LIST
		Vector deleteList3 = new Vector();
		deleteList3.addElement(new String("?m goto ?n"));
		Operator operator3 = new Operator(name3, ifList3, addList3, deleteList3);
		operators.addElement(operator3);

		// OPERATOR 4：お茶を買う
		/// NAME
		String name4 = new String("?m buy ?y for ?z");
		/// IF
		Vector ifList4 = new Vector();
		ifList4.addElement(new String("?m buy ?y for ?z"));
		/// ADD-LIST
		Vector addList4 = new Vector();
		addList4.addElement(new String("?m put ?y into ?z"));
		/// DELETE-LIST
		Vector deleteList4 = new Vector();
		deleteList4.addElement(new String("?m buy ?y for ?z"));
		Operator operator4 = new Operator(name4, ifList4, addList4, deleteList4);
		operators.addElement(operator4);

		// OPERATOR 5：お茶を冷蔵庫に置く
		/// NAME
		String name5 = new String("?m put ?y into ?z");
		/// IF
		Vector ifList5 = new Vector();
		ifList5.addElement(new String("?m put ?y into ?z"));
		/// ADD-LIST
		Vector addList5 = new Vector();
		addList5.addElement(new String("?z has ?y"));
		/// DELETE-LIST
		Vector deleteList5 = new Vector();
		deleteList5.addElement(new String("?z empty"));
